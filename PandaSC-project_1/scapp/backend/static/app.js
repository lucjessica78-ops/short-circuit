(() => {
  ...
  const state = {
    tool: "select",
    network: { buses: [], lines: [], trafos: [], ext_grids: [], loads: [], motors: [], generators: [] },
  ...
  function render() {
    ...
    net.generators.forEach(g => renderGenerator(g, busById(g.bus)));
  ...
  function renderBranch(el, kind, fromB, toB) {
    ...
    label.textContent = kind === "trafo" ? (el.std_type || "Trafo") : `${el.cable_label || "Cable"}, ${el.length_km} km`;
  ...
  function renderGenerator(gen, bus) {
    if (!bus) return;
    const x = bus.x - 48, y = bus.y - 10;
    const grp = svgEl("g", {});
    grp.appendChild(svgEl("line", { x1: bus.x - 16, y1: bus.y, x2: x + 12, y2: y, class: "motor-glyph" }));
    grp.appendChild(svgEl("circle", { cx: x, cy: y, r: 12, class: "motor-glyph" }));
    const label = svgEl("text", { x, y: y + 4, "text-anchor": "middle", class: "bus-sub" });
    label.textContent = "G";
    label.setAttribute("fill", "var(--copper)");
    grp.appendChild(label);
    grp.addEventListener("click", (evt) => { evt.stopPropagation(); onElementClick("gen", gen.id); });
    svg.appendChild(grp);
  }
  ...
  const TOOL_HINTS = {
    select: "Drag buses to move them. Click an element to see and edit its properties.",
    bus: "Click anywhere on the canvas to place a bus.",
    ext_grid: "Click a bus to attach an external grid (fault in-feed) there.",
    line: "Click a bus to start a cable, then click another bus to finish it.",
    ...
    generator: "Click a bus to attach a generator (contributes fault current).",
  ...
  function onBusClick(bus) {
    if (state.tool === "select") {
      ...
      openMotorForm(bus);
    } else if (state.tool === "generator") {
      openGeneratorForm(bus);
  ...
  function handleConnectClick(bus) {
    ...
    if (!state.pending || state.pending.kind !== kind) {
      ...
      setStatus(`${kind === "line" ? "Cable" : "Transformer"}: pick the second bus.`);
      render();
      return;
    }
    if (state.pending.fromBus === bus.id) return;
    if (kind === "line") {
  ...
  let cableChoicesCache = null;

  function jsonHeaders() { return { "Content-Type": "application/json" }; }

  async function getCableChoices() {
    if (!cableChoicesCache) {
      cableChoicesCache = await api("/api/cable_types");
    }
    return cableChoicesCache;
  }

  function cableOptionsHtml(choices, selectedKey) {
    return choices.map(c => `<option value="${c.key}" ${c.key === selectedKey ? "selected" : ""}>${c.label}</option>`).join("");
  }
  ...
  function deleteButtonHtml(type, id) {
    return `<button type="button" class="btn btn-danger" id="delete-selected">Delete</button>`;
  }

  function wireDelete(type, id) {
    const btn = document.getElementById("delete-selected");
    if (btn) btn.onclick = () => deleteElement(type, id);
  }
  ...
  async function openLineForm(fromBus, toBus) {
    let choices = [];
    try { choices = await getCableChoices(); } catch (e) { setStatus("Could not load cable types."); }
    showForm(`
      <div class="props-title">Cable — bus ${fromBus} &rarr; bus ${toBus}</div>
      <div class="field"><label>Cable size / material</label><select name="cable_key">${cableOptionsHtml(choices)}</select></div>
      <div class="field"><label>Length (km)</label><input name="length_km" type="number" step="any" value="0.1"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Add cable</button>
        <button type="button" class="btn btn-ghost" id="cancel-form">Cancel</button>
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      ...
      try {
        const data = await api("/api/line", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({
          from_bus: fromBus, to_bus: toBus, cable_key: fd.get("cable_key"), length_km: parseFloat(fd.get("length_km"))
        }) });
        applyNetwork(data);
        setStatus("Cable added.");
        clearSelection();
      } catch (err) { setStatus("Error: " + err.message); }
    };
    document.getElementById("cancel-form").onclick = clearSelection;
  ...
  function openGeneratorForm(bus) {
    showForm(`
      <div class="props-title">Generator — ${bus.name}</div>
      <div class="field field-hint">A synchronous generator contributing fault current (IEC 60909). Values below are typical starting points -- use the machine's actual nameplate/datasheet values where possible.</div>
      <div class="field"><label>Rated active power (MW)</label><input name="p_mw" type="number" step="any" value="1.0"></div>
      <div class="field"><label>Rated apparent power (MVA)</label><input name="sn_mva" type="number" step="any" value="1.25"></div>
      <div class="field"><label>Subtransient reactance Xd" (p.u.)</label><input name="xdss_pu" type="number" step="any" value="0.15"></div>
      <div class="field"><label>Subtransient resistance Rd" (ohm)</label><input name="rdss_ohm" type="number" step="any" value="0.01"></div>
      <div class="field"><label>Rated power factor (cos phi)</label><input name="cos_phi" type="number" step="any" value="0.8"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Attach</button>
        <button type="button" class="btn btn-ghost" id="cancel-form">Cancel</button>
      </div>
    ...
    propsForm.onsubmit = async (e) => {
      ...
      const fd = new FormData(propsForm);
      try {
        const data = await api("/api/generator", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({
          bus: bus.id, p_mw: parseFloat(fd.get("p_mw")), sn_mva: parseFloat(fd.get("sn_mva")),
          vn_kv: bus.vn_kv, xdss_pu: parseFloat(fd.get("xdss_pu")), rdss_ohm: parseFloat(fd.get("rdss_ohm")),
          cos_phi: parseFloat(fd.get("cos_phi")),
        }) });
        applyNetwork(data);
        setStatus("Generator attached.");
        clearSelection();
      } catch (err) { setStatus("Error: " + err.message); }
    };
    document.getElementById("cancel-form").onclick = clearSelection;
  ...
  function selectElement(type, id) {
    ...
    if (type === "bus") openBusEditForm(el);
    else if (type === "ext_grid") openExtGridEditForm(el);
    else if (type === "line") openLineEditForm(el);
    else if (type === "trafo") openTrafoEditForm(el);
    else if (type === "load") openLoadEditForm(el);
    else if (type === "motor") openMotorEditForm(el);
    else if (type === "gen") openGeneratorEditForm(el);
  }

  function findElement(type, id) {
    const map = { bus: "buses", line: "lines", trafo: "trafos", ext_grid: "ext_grids",
                  load: "loads", motor: "motors", gen: "generators" };
    const arr = state.network[map[type]] || [];
    return arr.find(e => e.id === id);
  }

  function openBusEditForm(bus) {
    showForm(`
      <div class="props-title">Edit bus #${bus.id}</div>
      <div class="field"><label>Name</label><input name="name" value="${bus.name}"></div>
      <div class="field"><label>Nominal voltage (kV)</label><input name="vn_kv" type="number" step="any" value="${bus.vn_kv}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("bus", bus.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/bus/${bus.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          name: fd.get("name"), vn_kv: parseFloat(fd.get("vn_kv"))
        }) });
        applyNetwork(data);
        setStatus("Bus updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("bus", bus.id);
  }

  function openExtGridEditForm(g) {
    showForm(`
      <div class="props-title">Edit external grid #${g.id}</div>
      <div class="field"><label>S"sc max (MVA)</label><input name="s_sc_max_mva" type="number" step="any" value="${g.s_sc_max_mva}"></div>
      <div class="field"><label>R/X max</label><input name="rx_max" type="number" step="any" value="${g.rx_max}"></div>
      <div class="field"><label>S"sc min (MVA)</label><input name="s_sc_min_mva" type="number" step="any" value="${g.s_sc_min_mva}"></div>
      <div class="field"><label>R/X min</label><input name="rx_min" type="number" step="any" value="${g.rx_min}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("ext_grid", g.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/ext_grid/${g.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          s_sc_max_mva: parseFloat(fd.get("s_sc_max_mva")), rx_max: parseFloat(fd.get("rx_max")),
          s_sc_min_mva: parseFloat(fd.get("s_sc_min_mva")), rx_min: parseFloat(fd.get("rx_min")),
        }) });
        applyNetwork(data);
        setStatus("External grid updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("ext_grid", g.id);
  }

  async function openLineEditForm(l) {
    let choices = [];
    try { choices = await getCableChoices(); } catch (e) { setStatus("Could not load cable types."); }
    showForm(`
      <div class="props-title">Edit cable #${l.id} (bus ${l.from_bus} &rarr; bus ${l.to_bus})</div>
      <div class="field"><label>Cable size / material</label><select name="cable_key">${cableOptionsHtml(choices, l.cable_key)}</select></div>
      <div class="field"><label>Length (km)</label><input name="length_km" type="number" step="any" value="${l.length_km}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("line", l.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/line/${l.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          cable_key: fd.get("cable_key"), length_km: parseFloat(fd.get("length_km"))
        }) });
        applyNetwork(data);
        setStatus("Cable updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("line", l.id);
  }

  async function openTrafoEditForm(t) {
    let types = [];
    try { types = await api("/api/std_types/trafo"); } catch (e) { setStatus("Could not load transformer types."); }
    const options = types.map(ty => `<option value="${ty}" ${ty === t.std_type ? "selected" : ""}>${ty}</option>`).join("");
    showForm(`
      <div class="props-title">Edit transformer #${t.id} (HV bus ${t.hv_bus} &rarr; LV bus ${t.lv_bus})</div>
      <div class="field"><label>Transformer type</label><select name="std_type">${options}</select></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("trafo", t.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/trafo/${t.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          std_type: fd.get("std_type")
        }) });
        applyNetwork(data);
        setStatus("Transformer updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("trafo", t.id);
  }

  function openLoadEditForm(ld) {
    showForm(`
      <div class="props-title">Edit load #${ld.id}</div>
      <div class="field"><label>Active power (MW)</label><input name="p_mw" type="number" step="any" value="${ld.p_mw}"></div>
      <div class="field"><label>Reactive power (Mvar)</label><input name="q_mvar" type="number" step="any" value="${ld.q_mvar}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("load", ld.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/loads/${ld.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          p_mw: parseFloat(fd.get("p_mw")), q_mvar: parseFloat(fd.get("q_mvar"))
        }) });
        applyNetwork(data);
        setStatus("Load updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("load", ld.id);
  }

  function openMotorEditForm(m) {
    showForm(`
      <div class="props-title">Edit motor #${m.id}</div>
      <div class="field"><label>Rated power (MW)</label><input name="p_mw" type="number" step="any" value="${m.p_mw}"></div>
      <div class="field"><label>Locked-rotor current ratio</label><input name="lrc_pu" type="number" step="any" value="${m.lrc_pu}"></div>
      <div class="field"><label>R/X ratio</label><input name="rx" type="number" step="any" value="${m.rx}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("motor", m.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/motor/${m.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          p_mw: parseFloat(fd.get("p_mw")), lrc_pu: parseFloat(fd.get("lrc_pu")), rx: parseFloat(fd.get("rx"))
        }) });
        applyNetwork(data);
        setStatus("Motor updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("motor", m.id);
  }

  function openGeneratorEditForm(g) {
    showForm(`
      <div class="props-title">Edit generator #${g.id}</div>
      <div class="field"><label>Rated active power (MW)</label><input name="p_mw" type="number" step="any" value="${g.p_mw}"></div>
      <div class="field"><label>Rated apparent power (MVA)</label><input name="sn_mva" type="number" step="any" value="${g.sn_mva}"></div>
      <div class="field"><label>Subtransient reactance Xd" (p.u.)</label><input name="xdss_pu" type="number" step="any" value="${g.xdss_pu}"></div>
      <div class="field"><label>Subtransient resistance Rd" (ohm)</label><input name="rdss_ohm" type="number" step="any" value="${g.rdss_ohm}"></div>
      <div class="field"><label>Rated power factor (cos phi)</label><input name="cos_phi" type="number" step="any" value="${g.cos_phi}"></div>
      <div class="props-actions">
        <button type="submit" class="btn btn-primary">Save changes</button>
        ${deleteButtonHtml("gen", g.id)}
      </div>
    `);
    propsForm.onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(propsForm);
      try {
        const data = await api(`/api/generator/${g.id}`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({
          p_mw: parseFloat(fd.get("p_mw")), sn_mva: parseFloat(fd.get("sn_mva")),
          xdss_pu: parseFloat(fd.get("xdss_pu")), rdss_ohm: parseFloat(fd.get("rdss_ohm")),
          cos_phi: parseFloat(fd.get("cos_phi")),
        }) });
        applyNetwork(data);
        setStatus("Generator updated.");
      } catch (err) { setStatus("Error: " + err.message); }
    };
    wireDelete("gen", g.id);
  }
  ...
  function renderResultsTable(data) {
    ...
    const notesHtml = (data.notes && data.notes.length)
      ? data.notes.map(n => `<div class="results-note">${n}</div>`).join("")
